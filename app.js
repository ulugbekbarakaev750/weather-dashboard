...
const UI_TRANSLATIONS = {
    ru: {
        search_placeholder: "Введите город...", geo: "Моя геопозиция", loading: "Загрузка данных...",
        sun: "☀️ Солнце", uv_title: "🕶️ УФ-индекс", chart: "📈 Температура (24ч)", forecast: "📅 Прогноз на 5 дней",
        greeting: "Привет", welcome_desc: "Введи город или нажми на геолокацию.",
    ...
    uz: {
        search_placeholder: "Shahar nomini kiriting...", geo: "Mening geolokatsiyam", loading: "Ma'lumotlar yuklanmoqda...",
        sun: "☀️ Quyosh", uv_title: "🕶️ UF-indeks", chart: "📈 Harorat (24 soat)", forecast: "📅 5 kunlik ob-havo",
        greeting: "Salom", welcome_desc: "Shaharni kiriting yoki geolokatsiyani bosing.",
    ...
    en: {
        search_placeholder: "Enter city...", geo: "My location", loading: "Loading data...",
        sun: "☀️ Sun", uv_title: "🕶️ UV Index", chart: "📈 Temperature (24h)", forecast: "📅 5-day forecast",
        greeting: "Hello", welcome_desc: "Enter a city or tap location.",
...
function getUVDescription(uv) {
    if (uv <= 2) return { text: T.uv_low, color: 'text-green-300' };
    if (uv <= 5) return { text: T.uv_mod, color: 'text-yellow-300' };
    if (uv <= 7) return { text: T.uv_high, color: 'text-orange-300' };
    if (uv <= 10) return { text: T.uv_vhigh, color: 'text-red-300' };
    return { text: T.uv_ext, color: 'text-purple-300' };
}

function checkAlerts(windSpeed, uvIndex, weatherCode) {
    els.alerts.innerHTML = '';
    let hasAlerts = false;
    const createAlert = (title, desc, bgClass) => {
        hasAlerts = true;
        const div = document.createElement('div');
        div.className = `p-3 rounded-xl border border-white/20 ${bgClass} backdrop-blur-md flex items-start gap-3 shadow-sm`;
        div.innerHTML = `<span class="text-xl">⚠️</span><div><h4 class="font-bold text-sm text-white">${title}</h4><p class="text-xs text-white/80">${desc}</p></div>`;
        els.alerts.appendChild(div);
    };
    if (windSpeed > 50) createAlert(T.alert_wind_title, T.alert_wind_desc, 'bg-orange-500/30');
    if (uvIndex >= 8) createAlert(T.alert_uv_title, T.alert_uv_desc, 'bg-red-500/30');
    if ([95, 96, 99].includes(weatherCode)) createAlert(T.alert_storm_title, T.alert_storm_desc, 'bg-yellow-500/30');
...
function buildChart(hourlyData) {
    ...
    // Цвет графика адаптируется под фон
    const isSnow = document.body.classList.contains('weather-snow');
    const lineColor = isSnow ? '#4facfe' : '#ffffff';
    const bgColor = isSnow ? 'rgba(79, 172, 254, 0.2)' : 'rgba(255, 255, 255, 0.2)';
    const textColor = isSnow ? '#333333' : 'rgba(255, 255, 255, 0.8)';

    if (chart) chart.destroy();
    chart = new Chart(ctx, {
        type: 'line',
        data: { labels, datasets: [{ data: temps, borderColor: lineColor, backgroundColor: bgColor, borderWidth: 2, tension: 0.4, fill: true, pointRadius: 0 }] },
        options: {
...
function renderForecast(dailyData) {
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        card.className = 'flex flex-col items-center min-w-[65px] p-2 rounded-xl bg-black/10 dark:bg-white/10';
        card.innerHTML = `<span class="text-xs font-semibold mb-1">${T.days[new Date(date).getDay()]}</span><i class="wi ${getWeatherIcon(code)} text-2xl text-white drop-shadow mb-1"></i><span class="text-xs font-bold">${max}°</span><span class="text-xs text-secondary">${min}°</span>`;
        els.forecast.appendChild(card);
    }
}

// 🎨 ФУНКЦИЯ ДИНАМИЧЕСКОЙ СМЕНЫ ФОНА
function applyDynamicTheme(weatherCode, isDay) {
    const classes = document.body.className.split(' ').filter(c => !c.startsWith('weather-'));
    let themeClass = '';
    
    if (weatherCode === 0) themeClass = isDay ? 'weather-clear-day' : 'weather-clear-night';
    else if ([1, 2, 3].includes(weatherCode)) themeClass = isDay ? 'weather-cloudy-day' : 'weather-cloudy-night';
    else if ([45, 48].includes(weatherCode)) themeClass = 'weather-fog';
    else if ([51, 53, 55, 56, 57, 61, 63, 65, 66, 67, 80, 81, 82].includes(weatherCode)) themeClass = 'weather-rain';
    else if ([71, 73, 75, 77, 85, 86].includes(weatherCode)) themeClass = 'weather-snow';
    else if ([95, 96, 99].includes(weatherCode)) themeClass = 'weather-storm';
    else themeClass = isDay ? 'weather-clear-day' : 'weather-clear-night';
...
async function fetchWeather(lat, lon, cityName = null) {
    ...
    try {
        ...
        applyDynamicTheme(c.weather_code, c.is_day);
        ...
        els.icon.className = `wi ${getWeatherIcon(c.weather_code, c.is_day)} text-7xl text-yellow-300 drop-shadow-lg wi-animate`;
        ...
        els.uvIndex.className = `text-2xl font-extrabold ${uvInfo.color} drop-shadow`;
